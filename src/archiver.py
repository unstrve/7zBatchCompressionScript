...
def find_7z(custom_path: str = "") -> Optional[str]:
    if custom_path:
        p = Path(custom_path)
        if p.exists():
            return str(p)
...
def build_7z_command(
    ...
    cmd.append("-y")
    cmd.append(_norm_path(archive_path))
    for s in sources:
        cmd.append(_norm_path(s))
    return cmd


def run_7z(cmd: List[str], line_callback: Callable[[str], None] | None = None,
           proc_holder: list | None = None, cwd: str | None = None) -> int:
